...
int main()
{
  ...
  */
  ...
  rep(i,q) {
    ...
    cin >> type >> p >> q;
    if (type == 0) {
      grid[p][q] = (grid[p][q] + 1) % 2;
      // 上 pが一番上から2番目以降に位置するとき
      if (p > 0) grid[p-1][q] = (grid[p-1][q] + 1) % 2;
      // 下 pが一番下から２番目に位置する時
      if (p < h-1) grid[p+1][q] = (grid[p+1][q] + 1) % 2;
      // 左
      if (q > 0) grid[p][q-1] = (grid[p][q-1] + 1) % 2;
      // 右
      if (q < w-1) grid[p][q+1] = (grid[p][q+1] + 1) % 2;
    } else {
      int ans = 0;
      ans+=grid[p][q];
      // 条件判定に配列を入れると山障害になる
      if (p > 0) ans+=grid[p-1][q];
      if (p < h-1) ans+=grid[p+1][q];
      if (q > 0) ans+=grid[p][q-1];
      if (q < w-1) ans+=grid[p][q+1];
      cout << ans << endl;
    }
  }
  return 0;
